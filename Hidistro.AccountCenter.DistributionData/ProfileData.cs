...
{
	public class ProfileData : PersonalSubsiteProvider
	{
		...
		public override DbQueryResult GetBalanceDetails(BalanceDetailQuery query)
		{
			...
			else
			{
				...
				string text = ProfileData.BuildUnderlingBalanceDetailsQuery(query);
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(*) FROM distro_BalanceDetails WHERE DistributorUserId = @DistributorUserId AND InpourId = @InpourId");
			this.database.AddInParameter(sqlStringCommand, "DistributorUserId", System.Data.DbType.Int32, HiContext.Current.SiteSettings.UserId.Value);
		...
		public override bool AddInpourBlance(InpourRequestInfo inpourRequest)
		{
			...
			{
				System.Data.Common.DbCommand storedProcCommand = this.database.GetStoredProcCommand("ac_Underling_InpourRequest_Create");
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM distro_InpourRequest WHERE InpourId = @InpourId;");
			this.database.AddInParameter(sqlStringCommand, "InpourId", System.Data.DbType.String, inpourId);
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("DELETE FROM distro_InpourRequest WHERE InpourId = @InpourId");
			this.database.AddInParameter(sqlStringCommand, "InpourId", System.Data.DbType.String, inpourId);
		...
		public override void GetStatisticsNum(out int noPayOrderNum, out int noReadMessageNum, out int noReplyLeaveCommentNum)
		{
			...
			int num3 = 0;
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendFormat("SELECT COUNT(*) AS NoPayOrderNum FROM distro_Orders WHERE UserId = {0} AND OrderStatus = {1} AND DistributorUserId = {2};", HiContext.Current.User.UserId, 1, HiContext.Current.SiteSettings.UserId);
			...
			stringBuilder.AppendFormat("SELECT COUNT(*) AS NoReplyLeaveCommentNum FROM distro_ProductConsultations WHERE UserId = {0} AND ReplyUserId is not null AND ViewDate is null AND DistributorUserId={1};", HiContext.Current.User.UserId, HiContext.Current.SiteSettings.UserId);
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
		...
		{
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendFormat("update distro_ProductConsultations set ViewDate=getdate() WHERE UserId = {0} AND ReplyUserId is not null AND ViewDate is null AND DistributorUserId={1};", HiContext.Current.User.UserId, HiContext.Current.SiteSettings.UserId);
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT count(ShippingId) as Count FROM distro_ShippingAddresses WHERE  UserID = @UserID");
		...
		{
			System.Data.Common.DbCommand storedProcCommand = this.database.GetStoredProcCommand("ac_Underling_ShippingAddress_CreateUpdateDelete");
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO distro_ShippingAddresses(RegionId,UserId,ShipTo,Address,Zipcode,EmailAddress,TelPhone,CellPhone) VALUES(@RegionId,@UserId,@ShipTo,@Address,@Zipcode,@EmailAddress,@TelPhone,@CellPhone); SELECT @@IDENTITY");
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM distro_MemberGrades WHERE CreateUserId = @CreateUserId");
		...
		{
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendFormat("ParentUserId={0} AND ReferralUserId = {1}", HiContext.Current.SiteSettings.UserId.Value, HiContext.Current.User.UserId);
		...
		private static string BuildUnderlingBalanceDetailsQuery(BalanceDetailQuery query)
		{
			StringBuilder stringBuilder = new StringBuilder();

...
namespace Hishop.TransferManager
{
	internal class TransferContainer
	{
		...
		private static void AddToExportIndex(Type type_0, string filename, DataTable dtExporters)
		{
			ExportAdapter exportAdapter = Activator.CreateInstance(type_0) as ExportAdapter;
			DataRow dataRow = dtExporters.NewRow();
			dataRow["fullName"] = type_0.FullName.ToLower();
		...
		private static void AddToImportIndex(Type type_0, string filename, DataTable dtImporters)
		{
			ImportAdapter importAdapter = Activator.CreateInstance(type_0) as ImportAdapter;

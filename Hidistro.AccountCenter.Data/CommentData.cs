...
namespace Hidistro.AccountCenter.Data
{
	public class CommentData : CommentMasterDataProvider
	{
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_Favorite(ProductId, UserId, Tags, Remark)VALUES(@ProductId, @UserId, @Tags, @Remark)");
		...
		public override DbQueryResult GetFavorites(int userId, string tags, Pagination page)
		{
			...
			System.Data.Common.DbCommand storedProcCommand = this.database.GetStoredProcCommand("ac_Member_Favorites_Get");
		...
		{
			bool result = false;
			string text = "DELETE from Hishop_Favorite WHERE FavoriteId IN (" + string_0 + ")";
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT COUNT(ReviewId) AS Count FROM Hishop_ProductReviews WHERE UserId=@UserId");
			this.database.AddInParameter(sqlStringCommand, "UserId", System.Data.DbType.Int32, HiContext.Current.User.UserId);
			int result = 0;
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
		...
		{
			System.Data.Common.DbCommand storedProcCommand = this.database.GetStoredProcCommand("ac_Member_ConsultationsAndReplys_Get");
			this.database.AddInParameter(storedProcCommand, "PageIndex", System.Data.DbType.Int32, query.PageIndex);
			this.database.AddInParameter(storedProcCommand, "PageSize", System.Data.DbType.Int32, query.PageSize);
			this.database.AddInParameter(storedProcCommand, "IsCount", System.Data.DbType.Boolean, query.IsCount);
		...
		public override DbQueryResult GetBatchBuyProducts(ProductQuery query)
		{
			...
			stringBuilder2.AppendFormat(" (CASE WHEN (SELECT COUNT(*) FROM Hishop_SKUMemberPrice WHERE SkuId = s.SkuId AND GradeId = {0}) = 1", member.GradeId);
			stringBuilder2.AppendFormat(" THEN (SELECT MemberSalePrice FROM Hishop_SKUMemberPrice WHERE SkuId = s.SkuId AND GradeId = {0})", member.GradeId);
			stringBuilder2.AppendFormat(" ELSE SalePrice * {0} /100 END) AS SalePrice", memberDiscount);
			return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "vw_Hishop_ProductSkuList s", "SkuId", stringBuilder.ToString(), stringBuilder2.ToString());
		}
		...
		{
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendFormat(" SELECT FavoriteId FROM Hishop_Favorite WHERE UserId = {0} ", userId);
			stringBuilder.AppendFormat(" AND ProductId IN (SELECT ProductId FROM Hishop_Products WHERE SaleStatus=1) ", new object[0]);
			if (!string.IsNullOrEmpty(tags))
			{
				stringBuilder.AppendFormat(" AND (ProductId IN (SELECT ProductId FROM Hishop_Products WHERE SaleStatus=1 AND ProductName LIKE '%{0}%') ", DataHelper.CleanSearchString(tags));

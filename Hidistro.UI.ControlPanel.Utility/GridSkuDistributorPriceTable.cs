...
namespace Hidistro.UI.ControlPanel.Utility
{
	public class GridSkuDistributorPriceTable : WebControl
	{
		protected override void Render(HtmlTextWriter writer)
		{
			...
			{
				DataTable skuDistributorPrices = ProductHelper.GetSkuDistributorPrices(text);
				if (skuDistributorPrices != null && skuDistributorPrices.Rows.Count > 0)
				{
					...
					for (int i = 7; i < skuDistributorPrices.Columns.Count; i++)
					{
						...
						stringBuilder.AppendFormat("<th class=\"td_right td_left\" scope=\"col\" style=\"width:100px\">{0}</th>", text2);
					}
					stringBuilder.AppendLine("</tr>");
					foreach (DataRow dataRow_ in skuDistributorPrices.Rows)
					{
		...
		private void CreateRow(DataRow dataRow_0, DataTable dtSkus, StringBuilder stringBuilder_0)
		{
			...
			for (int i = 7; i < dtSkus.Columns.Count; i++)
			{
				...
				stringBuilder_0.AppendFormat("<td><input type=\"text\" id=\"{0}_tdDistributorPrice_{1}\" name=\"tdDistributorPrice_{1}\" style=\"width:50px\" value=\"{2}\" />{3}</td>", new object[]
				{

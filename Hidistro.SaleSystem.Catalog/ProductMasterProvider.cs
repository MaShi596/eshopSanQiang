...
namespace Hidistro.SaleSystem.Catalog
{
	...
	{
		private static readonly ProductMasterProvider _defaultInstance;
		...
		{
			return ProductMasterProvider._defaultInstance;
		}
		...
		protected static string BuildUnSaleProductBrowseQuerySearch(ProductBrowseQuery query)
		{
			...
			if (!string.IsNullOrEmpty(query.TagIds))
			{
				...
				{
					string arg = array2[i];
					stringBuilder.AppendFormat(" and ProductId IN (SELECT ProductId FROM Hishop_ProductTag WHERE TagId={0})", arg);
				}
			}
			return stringBuilder.ToString();
		}

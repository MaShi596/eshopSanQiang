...
{
	public class EditMyOrder : DistributorPage
	{
		...
		private void grdProducts_RowDeleting(object sender, System.Web.UI.WebControls.GridViewDeleteEventArgs e)
		{
			...
			if (SubsiteSalesHelper.DeleteOrderProduct(string_, this.order))
			{
				this.order = SubsiteSalesHelper.GetOrderInfo(this.orderId);
				this.BindProductList(this.order);
				this.BindTatolAmount(this.order);
		...
		private void BindOtherAmount(OrderInfo order)
		{
			...
			if (order.Tax > 0m)
			{
				...
				System.Web.UI.WebControls.Literal expr_200 = this.litTax;
				expr_200.Text += "</span></td></tr>";
			}
			if (order.InvoiceTitle.Length > 0)
			{
				...
				System.Web.UI.WebControls.Literal expr_244 = this.litInvoiceTitle;
				expr_244.Text += "</span></td></tr>";
			}
		}

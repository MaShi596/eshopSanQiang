...
{
	public class Supplier_Admin_EditProduct : ProductBasePage
	{
		...
		protected System.Web.UI.HtmlControls.HtmlInputHidden hid_salPrice;
		protected System.Web.UI.HtmlControls.HtmlInputHidden hid_purchasePrice;
		...
		private void btnRemark_Click(object sender, System.EventArgs e)
		{
			...
			if (dataTable.Rows.Count > 0 && !string.IsNullOrEmpty(this.txtCostPrice.Text))
			{
				this.txtSalePrice.Text = System.Convert.ToDouble(decimal.Parse(dataTable.Rows[0]["SalePrice"].ToString()) * decimal.Parse(this.txtCostPrice.Text) / 100m).ToString("0.00");
				this.txtPurchasePrice.Text = System.Convert.ToDouble(decimal.Parse(dataTable.Rows[0]["PurchasePrice"].ToString()) * decimal.Parse(this.txtCostPrice.Text) / 100m).ToString("0.00");
				this.txtLowestSalePrice.Text = System.Convert.ToDouble(decimal.Parse(dataTable.Rows[0]["LowestSalePrice"].ToString()) * decimal.Parse(this.txtCostPrice.Text) / 100m).ToString("0.00");
			}
		}
				...
				dictionary = new System.Collections.Generic.Dictionary<string, SKUItem>
				{

					{
						...
						new SKUItem
						{
							...
							Weight = num5.HasValue ? num5.Value : 0
						}
					}
				};
		...
		private bool ValidateConverts(bool skuEnabled, out int displaySequence, out decimal purchasePrice, out decimal lowestSalePrice, out decimal salePrice, out decimal? costPrice, out decimal? marketPrice, out int stock, out int alertStock, out int? weight)
		{
			...
			if (!skuEnabled)
			{
				...
				{
					int value3;
					if (int.TryParse(this.txtWeight.Text, out value3))
					{
						weight = new int?(value3);
					}
					else
					{

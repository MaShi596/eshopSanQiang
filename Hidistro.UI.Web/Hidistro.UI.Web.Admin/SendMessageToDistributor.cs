...
{
	[PrivilegeCheck(Privilege.DistributorNewMsg)]
	public class SendMessageToDistributor : AdminPage
	{
		...
		private void btnRefer_Click(object sender, System.EventArgs e)
		{
			...
			if (this.userId == 0)
			{
				this.Page.Response.Redirect(Globals.GetAdminAbsolutePath("/comment/SendMessageSelectDistributor.aspx"));
				return;
			}
			if (this.userId > 0)
			{
				this.Page.Response.Redirect(Globals.GetAdminAbsolutePath(string.Format("/comment/SendMessageSelectDistributor.aspx?UserId={0}", this.userId)));
			}
		}
		private bool ValidateValues()
		{
			...
			{
				text += Formatter.FormatErrorMessage("内容不能为空，长度限制在1-60个字符内");
			}

...
namespace Hidistro.Core
{
	public static class DataHelper
	{
		...
		public static DbQueryResult PagingByRownumber(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string string_0, string filter, string selectFields)
		{
			return DataHelper.PagingByRownumber(pageIndex, pageSize, sortBy, sortOrder, isCount, table, string_0, filter, selectFields, 0);
		}
		...
		private static string BuildRownumberQuery(string sortBy, SortAction sortOrder, bool isCount, string table, string string_0, string filter, string selectFields, int partitionSize)
		{
			...
			stringBuilder.AppendFormat("{0} {1}", string.IsNullOrEmpty(sortBy) ? string_0 : sortBy, sortOrder.ToString());
			...
			{
				stringBuilder.AppendFormat(";SELECT COUNT({0}) FROM {1} {2}", string.IsNullOrEmpty(sortBy) ? string_0 : sortBy, table, text);
			}
			return stringBuilder.ToString();
		}
		public static DbQueryResult PagingByTopsort(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string string_0, string filter, string selectFields)
		{
			DbQueryResult result;
			if (string.IsNullOrEmpty(table))
			{
			...
			else
			{
				...
				else
				{
					...
					string text = DataHelper.BuildTopQuery(pageIndex, pageSize, sortBy, sortOrder, isCount, table, string_0, filter, selectFields);
					DbQueryResult dbQueryResult = new DbQueryResult();
					Database database = DatabaseFactory.CreateDatabase();
					System.Data.Common.DbCommand sqlStringCommand = database.GetSqlStringCommand(text);
					using (System.Data.IDataReader dataReader = database.ExecuteReader(sqlStringCommand))
		...
		private static string BuildTopQuery(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string string_0, string filter, string selectFields)
		{
			string text = string.IsNullOrEmpty(sortBy) ? string_0 : sortBy;
		...
		public static DbQueryResult PagingByTopnotin(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string string_0, string filter, string selectFields)
		{
			DbQueryResult result;
			if (string.IsNullOrEmpty(table))
			{
			...
			else
			{
				...
				else
				{
					...
					string text = DataHelper.BuildNotinQuery(pageIndex, pageSize, sortBy, sortOrder, isCount, table, string_0, filter, selectFields);
					DbQueryResult dbQueryResult = new DbQueryResult();
					Database database = DatabaseFactory.CreateDatabase();
					System.Data.Common.DbCommand sqlStringCommand = database.GetSqlStringCommand(text);
					using (System.Data.IDataReader dataReader = database.ExecuteReader(sqlStringCommand))
		...
		public static string BuildNotinQuery(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string string_0, string filter, string selectFields)
		{
			string text = string.IsNullOrEmpty(filter) ? "" : ("WHERE " + filter);
			...
			if (isCount)
			{
				stringBuilder.AppendFormat(";SELECT COUNT({0}) FROM {1} {2}", string_0, table, text);
			}
			return stringBuilder.ToString();
		}
		public static bool SwapSequence(string table, string keyField, string sequenceField, int int_0, int replaceKey, int sequence, int replaceSequence)
		{

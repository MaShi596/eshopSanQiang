...
function prepareControls(isReset) {
    if (currentTypeId.length == 0) currentTypeId = "0";
    var postUrl = "Supplier_ProductAdd.aspx?isCallback=true&action=getPrepareData&timestamp=";
...
function AddAttributeValue(attributeId,valuename,userMode) {
    $.ajax({
        url: "Supplier_SpecificationAdd.aspx",
...
function prepareSkus() {
    ...
    tableHeader.append($("<td align=\"center\" style=\"width:130px;display:none;\"><em >*<\/em>一口价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:100px;display:none;\">成本价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:150px;display:none;\"><em >*<\/em>分销商采购价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:45px\"><em >*<\/em>库存<\/td>"));
    tableHeader.append($("<td align=\"right\" style=\"width:60px\"><em >*<\/em>警戒库存<\/td>"));
...
function showSkuValue(){
    ...
    for (index = 0; index < cellFields.length; index++) {
        ...
        if (!flag) {
            var btnLi = $(String.format("<li style=\"float:left;\" id='li_{0}'><\/li>", cellFields[index]));
            var btn = $(String.format("<input style='display:none;' type=button id='btnadd_{0}' value='添加规格值'>", cellFields[index]));
...
function createSalePriceCell(rowIndex) {
    var cell = createDisplayNoneCell();
    ...
    var btnSkuMemberPrice = $("<input type=\"button\" value=\"会员价\" style=\"display:none;\" onclick=\"editSkuMemberPrice(" + rowIndex + ");\" \/>");
...
function createPurchasePriceCell(rowIndex) {
    ...
    var btnSkuPurchasePrice = $("<input type=\"button\" value=\"分销等级价\" style=\"width:75px;display:none;\" onclick=\"editSkuDistributorPrice(" + rowIndex + ")\" \/>");
...
function createDisplayNoneCell() {
    ...
    $.each($(".SpecificationTr"), function (i, valitem) {
        ...
        if ($("#gradePurchasePrice_" + rowIndex).val().length > 0) {
            ...
            if ($.browser.msie) {
                xml = new ActiveXObject("Microsoft.XMLDOM");
                xml.async = false;
                xml.loadXML($("#gradePurchasePrice_" + rowIndex).val());
            }
            else {
                xml = new DOMParser().parseFromString($("#gradePurchasePrice_" + rowIndex).val(), "text/xml");
            }

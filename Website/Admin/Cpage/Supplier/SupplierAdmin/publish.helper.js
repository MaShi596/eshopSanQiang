...
function GetCategories(parentCategoryId) {
    var categories = null;
    var postUrl = "Supplier_ProductSelectCategory.aspx?isCallback=true&action=getlist&timestamp=";
...
//        form.action = "supplier_productedit.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
//    else
    //        form.action = "Supplier_ProductAdd.aspx?categoryId=" + selectedCategoryId;

    if (productId != "" && productId.length > 0)
        //form.action = "editproduct.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
        window.location.href = "supplier_productedit.aspx?categoryId=" + selectedCategoryId + "&productId=" + productId;
    else {
        //form.action = "addproduct.aspx?categoryId=" + selectedCategoryId;
        window.location.href = "Supplier_ProductAdd.aspx?categoryId=" + selectedCategoryId;
    }
    return false;
    //form.submit();
}
...
function LoadState(categoryId) {
    $.ajax({
        url: String.format("Supplier_ProductSelectCategory.aspx?isCallback=true&action=getinfo&timestamp={0}&categoryId={1}", new Date().getTime(), categoryId),
        type: 'GET', dataType: 'json', timeout: 10000,
        async: false,
        success: function(resultData) {
            if (resultData.Status == "OK") {

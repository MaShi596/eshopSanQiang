...
function editMemberPrice(holderId, basePrice) {
    ...
    var postUrl = "Supplier_ProductAdd.aspx?isCallback=true&action=getMemberGradeList&timestamp=";
...
function editDistributorPrice(holderId, basePrice) {
    priceValueHolder = $("#" + holderId);
    var postUrl = "Supplier_ProductAdd.aspx?isCallback=true&action=getDistributorGradeList&timestamp=";
    postUrl += new Date().getTime() + "&typeId=" + currentTypeId;
...
function editProductDistributorPrice() {
    editDistributorPrice("ctl00_contentHolder_txtDistributorPrices", getPurchasePrice());
}

function editSkuDistributorPrice(rowIndex) {
    editDistributorPrice("gradePurchasePrice_" + rowIndex, getPrice($("#purchasePrice_" + rowIndex).val()));
}
...
function getPurchasePrice() {
    return getPrice($("#ctl00_contentHolder_txtPurchasePrice").val());
}

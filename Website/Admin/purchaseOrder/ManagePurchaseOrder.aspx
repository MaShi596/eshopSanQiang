...
<asp:Content ID="Content1" ContentPlaceHolderID="headHolder" runat="server">
                <script type="text/javascript">
                    function showOrderMatchItem(auto, status) {

                        if (auto != undefined && auto != "" && status == "2") {
                            var currentDate1 = new Date();
                            var ptUrl = applicationPath + "/Admin/CPage/Supplier/ShipPurchaseOrdersMatch.aspx?PurchaseOrderId=" + auto + "&t=" + currentDate1.getTime();
                            $("#htmlIframeMatchPts_" + auto).attr({ "src": ptUrl });
                        }
                    }
                    function showOrderItem(auto) {
                        ...
                        if (auto != undefined && auto != "") {
                            var currentDate1 = new Date();
                            var ptUrl = applicationPath + "/CPage/Supplier/ShipOrders.aspx?orderid=" + auto + "&t=" + currentDate1.getTime();
...
<div class="dataarea mainwidth">
	  ...
	  <ItemTemplate>	   
	        <tr class="td_bg">
		      <td style="background:#FF9900;color:#010101;"><input name="CheckBoxGroup" type="checkbox" value='<%#Eval("PurchaseOrderId") %>'>采购单编号：<%#Eval("PurchaseOrderId") %>
                <asp:Literal runat="server" Text='<%# String.IsNullOrEmpty(Eval("ShipOrderNumber").ToString()) ? "" : "<br>物流单编号：" + Eval("ShipOrderNumber")%>'></asp:Literal></td>
		      <td style="background:#FEC26A;color:#010101;">提交时间：<Hi:FormatedTimeLabel ID="lblStartTimes" Time='<%#Eval("PurchaseDate") %>' ShopTime="true" runat="server" ></Hi:FormatedTimeLabel></td>
		      <td style="background:#FCD396;color:#010101;"><%# (bool)Eval("IsPrinted")?"已打印":"未打印" %></td>
		      <td style="background:#FCD396;color:#010101;"><%#Eval("PaymentType") %>&nbsp;</td>
		      <td style="background:#FCD396;color:#010101;"><Hi:PuchaseStatusLabel runat="server" ID="lblPurchaseStatus" PuchaseStatusCode='<%# Eval("PurchaseStatus") %>'  /></td>
		      <td align="right" style="background:#FCD396;color:#010101;"><a href="javascript:RemarkPurchaseOrder('<%#Eval("PurchaseOrderId") %>','<%#Eval("OrderId") %>','<%#Eval("PurchaseDate") %>','<%#Eval("PurchaseTotal") %>','<%#Eval("ManagerMark") %>','<%#  Eval("ManagerRemark") %>')"><Hi:OrderRemarkImage runat="server" DataField="ManagerMark" ID="OrderRemarkImageLink" /></a></td>
	        </tr>  
		    <tr>
		      ...
		      <td>&nbsp;
                <a href="javascript:ClosePurchaseOrder('<%#Eval("PurchaseOrderId") %>');"><asp:Literal runat="server" ID="litClosePurchaseOrder" Visible="false" Text="关闭采购单" /></a> 
		          <asp:Label CssClass="submit_faihuo" id="lkbtnSendGoods" Visible="false" runat="server" >
                          <a style="color:Red" href="javascript:DialogFrame('<%# "Cpage/Supplier/Supplier_Admin_POrderOption.aspx?PurchaseOrderId="+ Eval("PurchaseOrderId") %>','生成发货单',null,null)">发货</a>
                   </asp:Label>                   
                ...
                DeleteMsg="确认要完成该采购单吗？" Visible="false" ForeColor="Red" />&nbsp;
                 <a style="color: Red"></a><a href="javascript:void(0)" onclick="return CheckRefund(this.title)"
                                ...
                                <a href="javascript:void(0)" onclick="return CheckReplace(this.title)" runat="server"
                                    ...
                                    <Hi:ImageLinkButton ID="lkbtnOrderMatch" IsShow="true" runat="server" Text="默认匹配发货" CommandArgument='<%# Eval("PurchaseOrderId") %>' CommandName="Match_Order"  DeleteMsg="确认要按默认匹配发货吗？" Visible="false" />
		      </td>
	        </tr>
            <tr style='<%#Eval("PurchaseStatus").ToString()=="2"?"": "display:none;" %>'>
            <td colspan="6">
             <iframe id='<%# "htmlIframeMatchPts_" + Eval("PurchaseOrderId") %>'  width="100%" scrolling="no" frameborder="0"  ></iframe>
                <%# "<script>showOrderMatchItem('" + Eval("PurchaseOrderId") + "','" + Eval("PurchaseStatus") + "')</script>"%>
            </td>
            </tr>
            <tr style='<%#Eval("IsPrinted").ToString()=="False"?"display:none;":"" %>'>
            <td colspan="6">
                <iframe id='<%# "htmlIframePts_" + Eval("PurchaseOrderId") %>' width="100%" scrolling="no" frameborder="0"  ></iframe>
                <%# "<script>showOrderItem('" + Eval("PurchaseOrderId") + "')</script>"%>
            </td>
            </tr>
	   </ItemTemplate>
	   <FooterTemplate>

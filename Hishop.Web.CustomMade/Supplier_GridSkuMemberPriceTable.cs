...
{
	public class Supplier_GridSkuMemberPriceTable : WebControl
	{
		protected override void Render(HtmlTextWriter writer)
		{
			...
			{
				System.Data.DataTable skuMemberPrices = ProductHelper.GetSkuMemberPrices(text);
				if (skuMemberPrices != null && skuMemberPrices.Rows.Count > 0)
				{
					...
					for (int i = 7; i < skuMemberPrices.Columns.Count; i++)
					{
						...
						stringBuilder.AppendFormat("<th class=\"td_right td_left\" scope=\"col\" style=\"width:100px;display:none\">{0}</th>", text2).AppendLine();
					}
					stringBuilder.AppendLine("</tr>");
					foreach (System.Data.DataRow dataRow_ in skuMemberPrices.Rows)
					{
		...
		private void CreateRow(System.Data.DataRow dataRow_0, System.Data.DataTable dtSkus, StringBuilder stringBuilder_0)
		{
			...
			for (int i = 7; i < dtSkus.Columns.Count; i++)
			{
				...
				stringBuilder_0.AppendFormat("<td style=\"display:none;\"><input type=\"text\" id=\"{0}_tdMemberPrice_{1}\" name=\"tdMemberPrice_{1}\" style=\"width:50px\" value=\"{2}\" />{3}</td>", new object[]
				{
